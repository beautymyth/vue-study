...
// 如下这些帮助程序可以在JS引擎中生成更好的vm代码，这是因为它们的明确性和函数内联性
 ...
 * dep(dependency-数据依赖)是可观察的，可以被多个指令订阅
 */
 ...
 * obj：被观察的对象
 * key：对象的key
 * val：key对应的value，默认为obj[key]
 * customSetter：用户自定义setter方法
 * shallow:浅观察，不观察对象value中的子对象
 */
function defineReactive (
  ...
  //兼容已经定义的getter/setters
  var getter = property && property.get;
  if (!getter && arguments.length === 2) {
    //如果还未设置get且参数只有(obj,key)，初始化val
    val = obj[key];
  }
...
function eventsMixin (Vue) {
  ...
  Vue.prototype.$on = function (event, fn) {
    ...
    if (Array.isArray(event)) {
      ...
      // 通过使用在注册时标记的布尔标记而不是散列查找来优化"钩子:事件成本
...
function stateMixin (Vue) {
  ...
  // the object here.
  // 在使用object.defineproperty时，flow在直接声明定义对象时不知何故出现了问题，
  // 所以我们必须循序渐进地在这里建立对象
  var dataDef = {};
